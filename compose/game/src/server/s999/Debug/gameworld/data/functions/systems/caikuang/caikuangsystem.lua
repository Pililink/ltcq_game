...
local function getRewardInfo(actor)
	...
	if sys_var.doubleEndTime then
		local now = System.getNowTime()
		if now < sys_var.doubleEndTime then
			local reward2 = utils.table_clone(reward)
			LActor.mergeRewrd(reward, reward2)
		else
			sys_var.doubleEndTime = nil
		end
	end
...
_G.beginMiningDouble = function(now_t, duration)
	local sys_var = System.getStaticVar()
	if not sys_var then return end
	sys_var.doubleEndTime = now_t + duration

	local actors = System.getOnlineActorList()
	if actors ~= nil then
		for i =1,#actors do
			local npack = LDataPack.allocPacket(actors[i], Protocol.CMD_Activity, Protocol.sActivityCmd_Double)
			if npack == nil then 
				break 
			end
			LDataPack.writeByte(npack, 2)
			LDataPack.writeInt(npack, duration)
			LDataPack.flush(npack)
		end
	end
	noticemanager.broadCastNotice(521)
end
...
local function onLogin(actor)
	...
	if sys_var.doubleEndTime and sys_var.doubleEndTime > now then
		local npack = LDataPack.allocPacket(actor, Protocol.CMD_Activity, Protocol.sActivityCmd_Double)
		if npack == nil then 
			return 
		end
		LDataPack.writeByte(npack, 2)
		LDataPack.writeInt(npack, sys_var.doubleEndTime - now)
		LDataPack.flush(npack)
	end
end
