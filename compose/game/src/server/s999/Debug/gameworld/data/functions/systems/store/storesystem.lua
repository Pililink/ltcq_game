...
function getStoreVar(actor)
	...
	if var.store.dailyLimitBuy == nil then
		var.store.dailyLimitBuy = {}
	end
	return var.store
end
...
--下发每周商城的数据
local function sendDailyStoreData(actor)
	local dailyVar = getStoreVar(actor)
	local npack = LDataPack.allocPacket(actor, Protocol.CMD_Store, Protocol.sStoreCmd_DailyStoreData) -- 16-8
	if npack == nil then return end
	local count = 0
	local pos = LDataPack.getPosition(npack)
	LDataPack.writeShort(npack, count)
	for _,v in ipairs(WeekTimeConfig) do
		if dailyVar.dailyLimitBuy[v.itemId] then
			count = count + 1
			LDataPack.writeInt(npack, v.itemId)
			--print("物品ID："..(tonumber(v.itemId)).."购买次数:"..count)
			LDataPack.writeInt(npack, (dailyVar.dailyLimitBuy[v.itemId] or 0))
		end
	end
	local pos2 = LDataPack.getPosition(npack)
	LDataPack.setPosition(npack, pos)
...
function buyGoods(actor, storeType, goodsList)
	print("商品类型："..storeType)
	if (storeType == storecommon.itemStore) then
		itemShoreBuy(actor, goodsList)
	elseif (storeType == storecommon.dailyStore) then
		print("商店类型3")
		 dailyShoreBuy(actor, goodsList)
	else  
...
--每周商店购买
function dailyShoreBuy(actor, goodsList)
	--需要多少钱的汇总
	local yuanBao = 0
	local itemList = {}
	--local needSpace = 0
	...
	for _,tb in pairs(goodsList) do
		-- 获取每日商店配置
		local dailyConfig = storecommon.getGoodsConfig(storecommon.dailyStore, tb.goodsId)
		if (not dailyConfig) then return end
		print("每日商店配置不为空")
		--获取道具配置
	--	local itemConfig = ItemConfig[dailyConfig.itemId]
	--	if (itemConfig and itemConfig.type == ItemType_Equip) then
	--		needSpace = needSpace + 1
	--	end
	--	print("消耗背包空间："..needSpace)
		local count = tb.count --购买数量	
		if count <= (dailyConfig.weekLimit or 0) then
			table.insert(itemList, {itemId = dailyConfig.itemId, count = count})
			if not dailyConfig.ybPrice then 
				print("价格为空！")
				return 
			end
			yuanBao = count * dailyConfig.ybPrice 
			print("消耗元宝:"..yuanBao)
		end
	...
	print("当前元宝数量:"..(curYuanBao or 0))
	--先扣钱
	LActor.changeCurrency(actor, NumericType_YuanBao, -yuanBao,
		"daily store buy:"..tostring(itemList[1].itemId) .. ":".. tostring(itemList[1].count))

	print("daily store buy:"..tostring(itemList[1].itemId) .. ":".. tostring(itemList[1].count))

	actorevent.onEvent(actor, aeStoreCost, NumericType_YuanBao, yuanBao)

	--再发货
	for _,tb in pairs(itemList) do
		print("发送的物品ID："..(tb.itemId or 0))
		LActor.giveAward(actor, AwardType_Item, tb.itemId, 1, "daily store buy")
		var.dailyLimitBuy[tb.itemId] = (var.dailyLimitBuy[tb.itemId] or 0) + tb.count
	end

	sendDailyStoreData(actor)
	--告诉前端购买成功
	reqBuyResult(actor)
end
...
function onNewDay(actor, login) -- login 今日是否主动登录了
	local storeVar = getStoreVar(actor)
	if (storeVar ~= nil) then
...
-- 每周初始化商店购买次数
local function initDailyStoreCount(actor)
	if not actor then return end
	local dailyVar = getStoreVar(actor)
	if dailyVar.dailyLimitBuy then
		for _,v in ipairs(WeekTimeConfig) do
			if dailyVar.dailyLimitBuy[v.itemId] then
				dailyVar.dailyLimitBuy[v.itemId] = 0
			end
		end
		return true
	end
...
-- 每周重置商店购买次数
local function resetDailyStoreBuy()
	print("重置每周限购次数")
	local actors = System.getOnlineActorList()
	if actors then
		for i=1, #actors do 
			if true == initDailyStoreCount(actors[i]) then
				sendDailyStoreData(actors[i])
			end
		end
	end
end
