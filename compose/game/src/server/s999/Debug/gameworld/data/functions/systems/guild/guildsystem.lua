...
-- GM
function gmdochangeName(guild, name)
	...
	local oldGName = LGuild.getGuildName(guildid)
	...
	LGuild.setChangeNameCount(guildid, 1)
	local gId = LGuild.getGuildId(guildid)
	print("GM.guildsystem.dochangeName gid:"..gId..",name:"..name)
	if LGuild.changeName(guildid, name) then
		System.logCounter(0, "GM:", "300", "guild", tostring(gId), "old:"..oldGName.."|new:"..name, "changeName", "")
	end

end

--GM修改公告
function gmhandleChangeMemo(guild, packet)

	local guildid = LGuild.getGuildById(guild)
	if guildid == nil then
		log_actor(0, "guild is nil")
		return 
	end

	if System.getStrLenUtf8(packet) > MAX_MEMO_LEN then
		log_actor(guild, "memo len error")
		return 
	end

	local packet = System.filterText(packet)
	changeMemo(guildid, packet)
	notifyUpdateGuildInfo(guildid, UpdateType.dtGuildInfo)
end
...
function handleImpeach(actor, packet)
	...
	local guildPos = LActor.getGuildPos(actor)
	if guildPos < (GuildConfig.impeachLevel or smGuildTz) then
		print("pos limit")
		return -- 堂主及以上官员
	end
