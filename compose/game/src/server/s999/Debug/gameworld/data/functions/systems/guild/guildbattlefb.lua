...
function onGetRoleCount(actor)
	local npack = LDataPack.allocPacket(actor,Protocol.CMD_GuildBattle,Protocol.sGuildBattleCmd_GetRoleCount)
	if npack == nil then
		return
	end
	local handle = LActor.getFubenHandle(actor)
	local count = Fuben.getActorCount(handle)
...
netmsgdispatcher.reg(Protocol.CMD_GuildBattle, Protocol.cGuildBattleCmd_GetRoleCount, onGetRoleCount)
