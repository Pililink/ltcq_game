...
local function setAidToSt(aid, nst, idx, subidx) -- actorid,状态,坐标x,坐标y
	local svar = getSysData()
...
local function AllPromPk(_, st, noTimer)
	...
	local hasOnes = false
	local notFbPk = true
	for idx,pinfo in ipairs(promData) do -- svar.Prom[st]
...
--请求报名清单
local function onReqSignup(actor, packet)
	--获取静态变量
	local sysvar = getSysData(actor)

	--下发参赛者名单
	local npack = LDataPack.allocPacket(actor, Protocol.CMD_PeakRace, Protocol.sPeakRace_ShowSignup) -- 67-15
	if(not npack) then
		print("peakracesystem.onReqSignup npack is nil")
		return
	end
	local npos = LDataPack.getPosition(npack) -- 记录当前位置
	local nCount = 0
	LDataPack.writeInt(npack, nCount) -- 参赛玩家数量
	for actorid,v in pairs(sysvar.SignUpActor or {}) do
		local actorname = LActor.getActorName(actorid)
		LDataPack.writeString(npack,actorname)
		nCount = nCount + 1
	end
	local npos2 = LDataPack.getPosition(npack) -- 记录最终点
	LDataPack.setPosition(npack, npos) -- 设置指针至nCount位置
	LDataPack.writeInt(npack, nCount) -- 参赛玩家数量
	LDataPack.setPosition(npack, npos2) -- 还原最终点
	LDataPack.flush(npack)
end
...
local function initGlobalData()
    ...
    --客户端请求来的消息
	netmsgdispatcher.reg(Protocol.CMD_PeakRace, Protocol.cPeakRace_ReqSignUp, reqSignUp) --请求报名 巅峰赛季报名
	...
	netmsgdispatcher.reg(Protocol.CMD_PeakRace, Protocol.cPeakRace_ShowSignup, onReqSignup) --请求报名人员清单 67-15
...
gm.showsign = function(actor,args)
    local sysvar = getSysData(actor)
	if(not sysvar or not sysvar.SignUpActor) then
		print("sysvar or sysvar.SignUpActor is nil")
		return
	end
	local nCount = 0
	local strInfo = "参赛者名单:\n"
	for actorid,v in pairs(sysvar.SignUpActor) do
		local actorname = LActor.getActorName(actorid)
		strInfo = strInfo .. actorname .. "\n"
		nCount = nCount + 1
	end
	strInfo = "参赛人数:" .. nCount .. "\n".. strInfo
	LActor.sendTipmsg(actor, strInfo,4)
end


gm.clearall = function()
	clearSysData()
	local svar = getSysData()
	svar.curStartTime = nil
	svar.curStatus = nil
	svar.GlobalStartTime = nil
end
