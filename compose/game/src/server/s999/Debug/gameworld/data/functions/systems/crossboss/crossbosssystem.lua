...
function sendActorData(actor)
	...
	if(not var.flagBelongLeftCount) then
		var.flagBelongLeftCount = CrossBossBase.flagBelongCount
	end
	if(not var.bossBelongLeftCount) then
		var.bossBelongLeftCount = CrossBossBase.bossBelongCount
	end
	LDataPack.writeShort(npack, var.flagBelongLeftCount)
	LDataPack.writeShort(npack, var.bossBelongLeftCount)
	...
	if 0 == fbSrvId then -- 苍月岛
		...
		if fbSrvId == System.getServerId() then -- 进的跨服中本服的领地
			...
			if CrossBossBase.otherServerEnterId then 
				...
				noticemanager.broadCastNotice(CrossBossBase.otherServerEnterId, actorName, actorSrvId, fbSrvId)
			end
		end
	end
end
...
-- 添加跨服采集旗帜次数，某次跨天未能更新次数的BUG补偿道具
function addFlagCount(actor,itemid,count) --actor对象 道具id 数量 16旗帜
	print("addFlagCount count" .. count)
	local data = getCrossStaticData(actor)
	local conf = ItemConfig[itemid]
	if(nil == conf) then
		print("addCount fail unknown item,itemid:" .. itemid .. " actorid:".. LActor.getActorId(actor))
		return item.use_item_error_code.conf_nil
	end
	local canUseCount = math.ceil((CrossBossBase.flagBelongMaxCount - (data.flagBelongLeftCount or 0))/(conf.useArg))
	print("addFlagCount canUseCount" .. canUseCount)
	if(canUseCount <= 0) then
		print("canUseCount<=0")
        return item.use_item_error_code.not_use
	end
	if(count>canUseCount) then -- 超过了
		print("count>canUseCount count" .. count)
		count = canUseCount
	end
	print("sub count:" .. count)
	local bRet = LActor.consumeItem(actor, itemid, count, false,"use addflag itemid:" .. itemid) -- 先扣道具
	if(bRet == false) then
		print("actorid:" .. LActor.getActorId(actor) .. "use addflag fail itemid:%d count:%d",itemid,count)
		return item.use_item_error_code.not_use -- 扣除失败了也显示不可使用
	end
	data.flagBelongLeftCount = (data.flagBelongLeftCount or 0) + count*conf.useArg
	if data.flagBelongLeftCount >= CrossBossBase.flagBelongMaxCount then
		data.flagBelongLeftCount = CrossBossBase.flagBelongMaxCount
	end
	sendActorData(actor)
	return item.use_item_error_code.use_succeed
end

-- 添加跨服BOSS归属次数，某次跨天未能更新次数的BUG补偿道具
function addCBossCount(actor,itemid,count) --actor对象 道具id 数量 17BOSS
	local data = getCrossStaticData(actor)
	local conf = ItemConfig[itemid]
	if(nil == conf) then
		print("addCount fail unknown item,itemid:" .. itemid .. " actorid:".. LActor.getActorId(actor))
	...
	local canUseCount = math.ceil((CrossBossBase.bossBelongMaxCount - (data.bossBelongLeftCount or 0))/(conf.useArg))
	if(canUseCount <= 0) then
        return item.use_item_error_code.not_use
	end
	if(count>canUseCount) then -- 超过了
	...
	local bRet = LActor.consumeItem(actor, itemid, count, false,"use addcboss itemid:" .. itemid) -- 先扣道具
	if(bRet == false) then
		print("actorid:" .. LActor.getActorId(actor) .. "use addcboss fail itemid:%d count:%d",itemid,count)
		return item.use_item_error_code.not_use
	end
	data.bossBelongLeftCount = (data.bossBelongLeftCount or 0) + count*conf.useArg
	if (data.bossBelongLeftCount > CrossBossBase.bossBelongMaxCount) then --useArg是系数 
			data.bossBelongLeftCount = CrossBossBase.bossBelongMaxCount
	end
	sendActorData(actor)
	return item.use_item_error_code.use_succeed
end
