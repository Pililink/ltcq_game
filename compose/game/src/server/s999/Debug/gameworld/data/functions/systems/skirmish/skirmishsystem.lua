	...
	for i = 1,SkirmishListSize do
		if data.actors[i] ~= nil then
			...
			if selfid == data.actors[i].id then
				...
				else -- 找不到小号的数据则用上面的方法伪装一下，避免获取不到数据卡死
					data.actors[i].id = selfid
					local level = data.actors[i].level + math.random(0, 3)
					if level % 1000 > MAX_ACTOR_LEVEL then -- 255
						level = math.floor(level /1000) * 1000 + MAX_ACTOR_LEVEL -- 直接满级 dmw
					end
					data.actors[i].level = level
				end
			end
