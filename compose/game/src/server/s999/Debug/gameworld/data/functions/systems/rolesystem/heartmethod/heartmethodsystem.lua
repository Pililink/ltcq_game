...
local function onReqDecomPose(actor, packet)
	local hmId = LDataPack.readShort(packet) -- 心法id
	local count = LDataPack.readInt(packet) -- 心法残页id的数量
	...
	local list = {}
	for i =1, count do
		local itemId = LDataPack.readInt(packet) -- 心法残页id
		list[itemId] = LDataPack.readInt(packet) -- 该id心法残页数量
	end
