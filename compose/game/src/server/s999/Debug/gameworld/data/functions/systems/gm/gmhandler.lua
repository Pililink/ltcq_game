...
gmCmdHandlers.deletechat = function ( actor, args )
	local aid = LActor.getActorId(actor)
	chat.delActorChat(aid)
	return true
end

gmCmdHandlers.sendRechargeRank = function ( actor, args )
	rechargerank.sendRankingReward(669, RankingType_Recharge, ActivityType4Config[669])
	return true
end

-- @setlvupevent 1 80 触发升级事件
gmCmdHandlers.setlvupevent = function ( actor, args )
	local lvbegin = tonumber(args[1])
	local lvend = tonumber(args[2])
	if(lvbegin<1 or lvend>80) then return false end
	for i=lvbegin,lvend do
		actorexp.onLevelUp(actor, i)
	end
	return true
end
...
-- 重置vip礼包领取次数
gmCmdHandlers.resetVip = function(actor, args)
	vip.resetVipCount(actor)
	return true
end
--[[
--@geninv 生成激活码
gmCmdHandlers.geninv = function ( actor, args )
	local actorId = LActor.getActorId(actor)
	local invCode = LActor.genInvCodeById(actorId)
	sendTip(actor,"您的邀请码是:" .. invCode ,4)
	return true
end

gmCmdHandlers.decodeinv = function ( actor, args )
	local invCode = args[1]
	local actorId = LActor.decodeInvCode(invCode)
	sendTip(actor,"您的角色id是:" .. actorId ,4)
	return true
end
]]
