	...
	for itemid, count in pairs(list) do
		...
		if conf and conf.gain then
			if LActor.getItemCount(actor, itemid) >= count then
				LActor.costItem(actor, itemid, count, "decomposeFuwen")
				LActor.changeCurrency(actor, NumericType_Shatter, conf.gain*count, "decomposeFuwen")
				if conf.chip and not compose then
					LActor.changeCurrency(actor, NumericType_SpeShatter, conf.chip*count, "decomposeFuwen")
				end
				if conf.goldCount and conf.goldCount > 0 then
					LActor.giveItem(actor, RuneOtherConfig.goldItemId, conf.goldCount*count, "decomposeFuwen")
				end

				gain = gain + conf.gain*count
			else
				print("[ERROR] DecomposeFuwen : count error; itemid:"..itemid.." cpcount: "..count.." rcount:"..LActor.getItemCount(actor, itemid) or 0)
			end
...
local function OnDecomposeFuwen(actor,packet)
	...
	for i =1, count do
		...
		list[item_id] = LDataPack.readInt(packet)
	end
	if not isOpen(actor) then
		LActor.sendTipmsg(actor, LAN.FuWen.fw007, ttScreenCenter)
		return
...
local function OnConverFuwen(actor,packet)
	...
	LActor.giveAward(actor, AwardType_Item, itemId, count, "byConver")
...
local function composeFuwen(actor, packet)
	...
	DecomposeFuwen(actor,count,costItem, true)
