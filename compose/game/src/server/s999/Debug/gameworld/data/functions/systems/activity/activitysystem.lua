...
require("systems.activity.subactivitytype24")
...
local function loadTime(conf)
    ...
    elseif conf.timeType == 3 then
		...
		local cur = System.getNowTime()
		local cur_Y, cur_M, cur_d, cur_h, cur_m, cur_s = System.timeDecode(cur)
		local cur_w = System.getWeekDataTime(cur_Y, cur_M, cur_d)

		print("activitysystem.loadTime (" .. conf.Id ..") Begin ======================== NOW: "..cur_Y.."-"..cur_M.."-"..cur_d.." "..cur_h..":"..cur_m..":"..cur_s)

		-- 获取配置时间
		local sw,sh,sm = string.match(conf.startTime, "(%d+)-(%d+):(%d+)")
		local ew,eh,em = string.match(conf.endTime, "(%d+)-(%d+):(%d+)")
		sw,sh,sm  = tonumber(sw),tonumber(sh),tonumber(sm)
		ew,eh,em  = tonumber(ew),tonumber(eh),tonumber(em)
		print("activitysystem.loadTime (" .. conf.Id ..") sw:"..sw.." "..sh..":"..sm.."  ew:"..ew.." "..eh..":"..em)

		-- 计算矫正后起始/结束时间
		local cur_timestamp = cur
		local open_timestamp = System.timeEncode(cur_Y, cur_M, cur_d + (sw - cur_w), sh, sm, 0) 
		local close_timestamp = System.timeEncode(cur_Y, cur_M, cur_d + (ew - cur_w), eh, em, 0)
		if close_timestamp < open_timestamp then
			open_timestamp = open_timestamp - 7 * utils.day_sec
		end

		-- 再次调整时间
		local st = open_timestamp
		local et = close_timestamp
		if cur_timestamp > close_timestamp then
			st = open_timestamp + 7 * utils.day_sec
			et = close_timestamp + 7 * utils.day_sec
		end

		local Y,M,D,h,m,s = System.timeDecode(st)
		print("activitysystem.loadTime (" .. conf.Id ..") start: "..Y.."-"..M.."-"..D.." "..h..":"..m..":"..s)
		Y,M,D,h,m,s = System.timeDecode(et)
		print("activitysystem.loadTime (" .. conf.Id ..") end: "..Y.."-"..M.."-"..D.." "..h..":"..m..":"..s)
...
local function checkOpenTimeLt(conf)
	...
	if(conf.openTimeType and conf.openTimeType == 0) then -- 存在则按开服时间进行判断
		local d,h,m = string.match(conf.openTimeLt, "(%d+)-(%d+):(%d+)")
		local srvopenday = System.getOpenServerDay() + 1
		if(srvopenday > tonumber(d)) then return false end -- 不满足要求
		return true
	end
...
local function checkOpenTimeGt(conf)
	...
	if(conf.openTimeType and conf.openTimeType == 0) then -- 存在则按开服时间进行判断
		local d,h,m = string.match(conf.openTimeGt, "(%d+)-(%d+):(%d+)")
		local srvopenday = System.getOpenServerDay() + 1
		if(srvopenday < tonumber(d)) then return false end -- 不满足要求
		return true
	end
...
local function loadConfig()
    ...
    for id, conf in pairs(ActivityConfig) do
		if not System.isCommSrv() or 
			...
			if et > System.getNowTime() or conf.timeType == 2 then
				activities[id] = {
...
local function onLogin(actor)
    ...
    for id, activity in pairs(globalData.activities) do
		...
		if now < activity.endTime then
			...
			if ActivityConfig[id] and ActivityConfig[id].timeType == 3 then
				loopActivityEndConf[id] = ActivityConfig[id]
			end
...
local function onUpdate()
	...
	for id,conf in pairs(loopActivityEndConf) do
		if not System.isCommSrv() or
			(checkOpenTimeLt(conf) and
			checkOpenTimeGt(conf) and
			checkHefuTimeLt(conf) and
			...
			checkHefuTimes(conf)
			then
			print("activitysystem.loadTime - onUpdate id:" .. id)
			local st, et, err = loadTime(conf)
			if err then print("onUpdate time err") return nil end
			local type= conf.activityType
			local typeConfig = subactivities.getConfig(type)
			if typeConfig == nil then print("onUpdate type config err.."..type) return nil end
			if typeConfig[id] == nil then print("onUpdate type config err.id:"..id) return nil end
			local varClear = false
			if conf.endClear == nil or conf.endClear == true then
				varClear = true
			end

			if et > System.getNowTime() then
				globalData.activities[id] = {
					id=id, startTime=st, endTime=et,
					type=conf.activityType,
					mark=conf.startTime..conf.endTime,
					varClear = varClear --就不应该有这破玩意
...
local function init()
    ...
    engineevent.regGameTimer(onUpdate)
end
