...
local function onSwitchTarget(actor,pack)
	local et_hdl = LDataPack.readInt64(pack) -- 玩家唯一标识
	if et_hdl == 0 then 
...
local function reqSwitchCamp(actor, cpacket)
	local campMode = LDataPack.readShort(cpacket)
	if(not campMode) then return end
	if(campMode==1) then
...
netmsgdispatcher.reg(Protocol.CMD_Base, Protocol.cBaseCmd_SwitchCamp,reqSwitchCamp)
