...
local function isLevelUp(actor)
	...
	log_print( LActor.getActorId(actor) .. " knighthood.isLevelUp: ok "..level)
	return true
end
...
local function onLevelUpKnighthood(actor, packet)
	if isLevelUp(actor) then 
		...
		local achievementIds = KnighthoodConfig[level].achievementIds
		for i,v in pairs(achievementIds or {}) do 
			log_print( LActor.getActorId(actor) .. " knighthood.onLevelUpKnighthood: achieveId:"..v.achieveId.." taskId:"..v.taskId)
		...
		SendKnighthoodData(actor)
		loadAttrs(actor)
		log_print( LActor.getActorId(actor) .. " knighthood.onLevelUpKnighthood: ok "..level)
	end
end
--net end
...
local function onLogin(actor)
	-- 这里是为了修复成就任务的一个bug
	local level = getKnighthoodLv(actor) - 1
	if level > 0 and level < #KnighthoodConfig then
		local achievementIds = KnighthoodConfig[level].achievementIds
		for i,v in pairs(achievementIds or {}) do 
			achievetask.checkAndFinish(actor,v.achieveId,v.taskId)
		end
	end
	SendKnighthoodData(actor)
end
