...
local function SendHaveReward(actor, type)
	...
	if sys_var.doubleExpEndTime then
		local now = System.getNowTime()
		if now < sys_var.doubleExpEndTime then
			expex = expex + 1
		else
			sys_var.doubleExpEndTime = nil
		end
	end
...
local function onReqWorldReward(actor,packet)
	...
	if (data.dq_rewardRecord[index] and data.dq_rewardRecord[index] == 1) then
		print("dq_rewardRecord error:"..data.dq_rewardRecord[index])
		return
	end
...
local function onLogin(actor)
	...
	local sys_var = System.getStaticVar()
	if not sys_var then return end
	if sys_var.doubleExpEndTime and sys_var.doubleExpEndTime > now then
		local npack = LDataPack.allocPacket(actor, Protocol.CMD_Activity, Protocol.sActivityCmd_Double)
		if npack == nil then
			return
		end
		LDataPack.writeByte(npack, 1)
		LDataPack.writeInt(npack, sys_var.doubleExpEndTime - now)
		LDataPack.flush(npack)
	end
end
...
_G.beginChapterDoubleExp = function(now_t, duration)
	local sys_var = System.getStaticVar()
	if not sys_var then return end
	sys_var.doubleExpEndTime = now_t + duration
	...
	noticemanager.broadCastNotice(520)
end
