...
-- 增加一个vip奖励领取记录
-- lastWeek  上次领取周礼包时间
-- extraExp 额外的vip经验（原本只以充值数计算vip等级）
...
local function onReqRewards(actor, packet)
    local level = LDataPack.readShort(packet) -- 要购买的等级
    ...
    --判断钱是否足够
    if VipConfig[level].needYb and VipConfig[level].needYb > 0 then--VIP礼包免费领取
        local yb = LActor.getCurrency(actor, NumericType_YuanBao)
        --if VipConfig[level].needYb1 > yb then 
        --    print(LActor.getActorId(actor).." vip.onBuyAward yuanbao not enough level:"..level..VipConfig[level].needYb1)
        --    return 
        --end
        --LActor.changeYuanBao(actor, 0 - VipConfig[level].needYb1, "action_buyVipGift_".. level)
    end
...
local function onLogin(actor)
    ...
    if LActor.getVipLevel(actor) == 0 and LoginActivateConfig.firstvip and LoginActivateConfig.firstvip > 0 then
        LActor.setVipLevel(actor, LoginActivateConfig.firstvip)
        updateVipAttr(actor)
    end
    ...
    LDataPack.writeInt(npack, LActor.getRecharge(actor) + (data.extraExp or 0))
...
function addVipExp(actor, val)
    ...
    if data.extraExp == nil then
        data.extraExp = 0
    end
    
    -- 增加额外vip经验
    data.extraExp = data.extraExp + val
    local extraExp = data.extraExp
    totalCharge = totalCharge + extraExp
    ...
    print("addVipExp actorid:" .. LActor.getActorId(actor) .. " vip:" .. level .. " add: " .. extraExp)
end
...
local function onCharge(actor, val)
    ...
    totalCharge = totalCharge + data.extraExp
