...
local function onReqEnterFuBen(actor, packet)
	...
	resetFbId(actor) -- 尝试重置挑战状态，这个只有在BOSS重新刷新的情况下才会生效，魔界入侵从一天1次改为一天2次2019-11-20
...
-- itemid：道具id count：数量，无论是多少都默认是1，挑战次数不会累计
function AddDevilBossCount(actor,itemid,count)
	local var = getCrossStaticData(actor)
	-- 判断是否可用 如未参与过提示不可用
	if(var == nil or (var.id == nil and var.scene_cd == nil)) then
		print("can not use itemid=" .. itemid)
		return item.use_item_error_code.not_use
	end
    -- 扣道具
	local bRet = LActor.consumeItem(actor, itemid, 1, false,"use AddDevilBossCount :" .. itemid) -- 先扣道具
	if(bRet == false) then
		print("actorid:" .. LActor.getActorId(actor) .. "use AddDevilBossCount fail itemid:%d count:1",itemid)
		return item.use_item_error_code.not_use
	end
	-- 刷新挑战次数
	var.id = nil
...
local function initGlobalData()
    ...
    --本服消息处理
    netmsgdispatcher.reg(Protocol.CMD_DevilBoss, Protocol.cDevilBossCmd_RequestEnter, onReqEnterFuBen) --75-5
    --netmsgdispatcher.reg(Protocol.CMD_DevilBoss, Protocol.cDevilBossCmd_ReqShowInfo, onReqShowInfo)

    --跨服消息处理(跨服服来的消息)
    csmsgdispatcher.Reg(CrossSrvCmd.SCDevilBossCmd, CrossSrvSubCmd.SCDevilBossCmd_RefreshBoss, onRefreshBoss) -- 10-1 魔界入侵BOSS刷新
