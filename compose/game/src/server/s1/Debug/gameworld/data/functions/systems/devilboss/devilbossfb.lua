...
--根据帮派人数获取拍卖id集合
local function getAuctionList(isBelong, guildId, conf, number) -- conf = conf.belongAuctionList
	local idList = {}

	if isBelong or conf then
		for _, info in ipairs(conf or {}) do
			if info.num[1] <= number and info.num[2] >= number then
				local count = math.random(info.count[1], info.count[2]) -- 抽取次数
				for i=1, count do
					local list = auctiondrop.dropGroup(info.dropId) -- 奖励物品表
...
local function sendGuildReward(id, guildId, idList, actors, isBelong, winSrvId, winSrvName)
	if not System.isCommSrv() then
		...
		LDataPack.writeInt(npack, guildId) -- 帮会id
		LDataPack.writeInt(npack, id) -- bossid
		local strlog = "devilboss award,bossid: " ..  id .. "itemidlist:" --跨服的日志，记录本次奖励道具与参与的玩家
		
		local srvId, name = getGuildServerInfo(id, guildId)
		LDataPack.writeInt(npack, isBelong and winSrvId or srvId) -- srvid
		LDataPack.writeString(npack, isBelong and winSrvName or name) -- srvname
		LDataPack.writeShort(npack, #(idList or {})) -- 奖励列表中的数量
		for _, id in pairs(idList or {}) do -- 奖励列表
			LDataPack.writeInt(npack, id)
			strlog = strlog .. id .. " "
		end
		
		strlog = strlog .. " actoridlist: "
		LDataPack.writeShort(npack, #(actors or {})) -- 参与玩家数量
		--print("参与玩家数量" .. #(actors or {}))
		for i=1, #(actors or {}) do 
			LDataPack.writeInt(npack, LActor.getActorId(actors[i]))
			strlog = strlog .. LActor.getActorId(actors[i]) .. " "
		end
        strlog = strlog .. " guildid:" .. guildId
...
local function sendReward(id, actor)
	...
	local data = getBossData(id)
	local actors = Fuben.getAllActor(data.fbHandle) -- 获取所有玩家
	if actors then
		for i = 1, #actors do
			local id = LActor.getGuildId(actors[i]) -- 获取帮会id
			if 0 ~= id then -- 有帮会
				if not haveGuildList[id] then haveGuildList[id] = {} end -- 集合没找到该帮会则添加
	...
	local winGuildId = LActor.getGuildId(actor) -- 归属者帮会id
	local winSrvId = LActor.getServerId(actor) -- 归属服务器id
	local winSrvName = LActor.getGuildName(actor) -- 归属服务器名称

	local conf = DevilBossConfig[id]
	local belongReward = drop.dropGroup(conf.belongReward) -- 归属奖
	local joinReward = drop.dropGroup(conf.joinReward) -- 参与奖

	--伤害排序
	sortRank(id, winGuildId) -- 按照伤害给其他非归属者帮会排名  data.damageRank dmw

	--帮派成员弹框通告
	for guildId, actors in pairs(haveGuildList or {}) do -- actors 帮会成员列表
		local isBelong = guildId == winGuildId

		local cfg = nil
		if not isBelong then cfg = conf.joinAuctionList[getRank(data.damageRank, guildId)] end -- 非boss归属者帮会参与奖励 看配置配几个帮会能有
		local idList = getAuctionList(isBelong, guildId, isBelong and conf.belongAuctionList or cfg, #(actors or {})) -- 帮会奖励itemid列表
		if 0 < #idList then sendGuildReward(id, guildId, idList, actors, isBelong, winSrvId, winSrvName) end -- 发送奖励到游戏服
...
local function createFb(id, curTime, isInit)
	...
	if not data.bossList[id] or data.bossList[id].iskill then
		...
		data.bossList[id].refreshTime = curTime -- 没死的BOSS更新一下刷新时间
		--print("boss没死 刷新时间")
	end
end
