...
gmCmdHandlers.godweapon = function(actor, args)
	...
	if a == 1 then -- 神兵总经验
		local expnum = tonumber(args[2]) or 0
		addGodWeaponExp(actor, expnum, "gm add godweapon exp" .. expnum)
		print("gm add godweapon exp" .. expnum)
	elseif a == 2 then
	...
	elseif a==10 then -- 神兵总经验置0
		local var = getActorVar(actor)
		var.exp = 0
		print("gm reset godweapon exp")
	elseif a == 11 then
		local var = getActorVar(actor)
        print("gm show exp" .. (var.exp or 0))
	end
