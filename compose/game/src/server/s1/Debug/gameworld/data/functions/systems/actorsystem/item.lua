...
local function checkBelongItem(actor,item_id,count) -- 检测添加跨服BOSS归属次数和旗帜采集次数的道具是否足够
	local conf = ItemConfig[item_id]
	if(conf == nil) then
		print("item's config is not exist,check ItemConfig,itemid: %d",item_id)
	...
	if not LActor.checkItemNum(actor,item_id,count,false) then 
		...
		return use_item_error_code.lazy_weight -- 数量不够
	end
...
-- 检测魔界入侵刷新道具是否可用
local function checkDeviBossItem(actor,item_id,count)
	local conf = ItemConfig[item_id]
	if(conf == nil) then
		print("item's config is not exist,check ItemConfig,itemid: %d",item_id)
		return use_item_error_code.conf_nil -- 缺配置
	end
	if not LActor.checkItemNum(actor,item_id,count,false) then 
		print(LActor.getActorId(actor) .. "checkDefault: lazy_weight " .. item_id )
		return use_item_error_code.lazy_weight -- 数量不够
	...
	print(LActor.getActorId(actor) .. "checkDefault: ok " .. item_id)
	return use_item_error_code.not_error -- 没问题
end
...
local function checkDefault(actor,item_id,count)
	...
	if level < needLevel then 
		...
		LActor.sendTipmsg(actor, string.format("等级不足,无法使用"), ttScreenCenter)
...
local function useDefault(actor,item_id,count)
	...
	if(item_id==1991001)
    then
        privilegemonthcard.buyPrivilegeMonth(LActor.getActorId(actor))
    elseif(item_id==1991000)
		then
...
local function useTitle(actor,item_id,count)
	local conf = ItemConfig[item_id]
	local args = conf.useArg -- 称号id
	if(titlesystem.isTitleAcquired(actor,args)) then
		-- 判断是否有过该称号
		return use_item_error_code.not_use
	end
	local ret = useDefault(actor,item_id,1) -- 称号只能使用一个
	if ret ~= use_item_error_code.use_succeed then 
		return ret
	end
...
local function useAddBossCount(actor, item_id, count)
	...
	-- 对类型5（神兵圣域）特殊处理
	if useArg.type == 5 then
		worldboss.AddBelongCount(actor, useArg.type, canUseCount * useArg.count)
	end
	return use_item_error_code.use_succeed
end
...
local function useAddPrivilegeMonthCard(actor, item_id, count)
	local conf = ItemConfig[item_id]
	local ret = useDefault(actor,item_id, count) 
	if ret ~= use_item_error_code.use_succeed then 
		return ret
...
local function useAddVipExp(actor, item_id, count)
	local conf = ItemConfig[item_id]
	local ret = useDefault(actor, item_id, count) 
	if ret ~= use_item_error_code.use_succeed then 
		return ret
	end
	local expVal = conf.useArg or 0;
	vip.addVipExp(actor, expVal * count)
	return use_item_error_code.use_succeed
end

local function useAddBossScore(actor, item_id, count)
	local conf = ItemConfig[item_id]
	local ret = useDefault(actor, item_id, count) 
	if ret ~= use_item_error_code.use_succeed then 
		return ret
	end
	actorevent.onEvent(actor, aeGetWBossActScore, conf.useArg * count)
	return use_item_error_code.use_succeed
end

local function useAddFlagCount(actor, item_id, count) -- 跨服旗帜次数增加
	local conf = ItemConfig[item_id]
	print("addflagcount count: " .. count)
	return crossbosssystem.addFlagCount(actor,item_id,count)
end


local function useAddCBossCount(actor, item_id, count) -- 跨服BOSS次数增加
	local conf = ItemConfig[item_id]
	return crossbosssystem.addCBossCount(actor,item_id,count)
end

-- 使用道具刷新魔界入侵个人挑战次数
local function useAddDevilBossCount(actor, itemid, count)
	return devilbosssystem.AddDevilBossCount(actor,itemid,count)
end

-- 使用代金券 增加等级使用限制
local function useVoucher(actor, itemid, count)
	local conf = ItemConfig[itemid]
	if(not conf) then
		print("[ERROR]:item.useVoucher config not exist,ItemConfig[" .. itemid .. "]") 
		return use_item_error_code.conf_nil
	end
	local needLevel = (conf.zsLevel * 1000) + conf.level
	local level = (LActor.getZhuanShengLevel(actor) * 1000) + LActor.getLevel(actor)
	if level < needLevel then 
		print(LActor.getActorId(actor) .. "checkDefault: insufficient_leve " .. itemid)
		LActor.sendTipmsg(actor, string.format("等级不足,无法使用"), ttScreenCenter)
		return use_item_error_code.insufficient_level
	end
	-- 是否首冲过
	--if(not dailyrecharge.hasRecharged(actor)) then
	--	LActor.sendTipmsg(actor, LAN.ScriptTips.gcn015)
	--	return use_item_error_code.not_use
	--end
   if level >=  needLevel then
	local ret = useDefault(actor, itemid, count) -- 消耗掉代金券
	if ret ~= use_item_error_code.use_succeed then 
		return ret
	end
	LActor.changeCurrency(actor, NumericType_YuanBao, conf.useArg.yb * count, string.format("useitem_%s_%s",itemid,count)) -- 增加元宝
	vip.addVipExp(actor, conf.useArg.vipexp * count) -- 增加vip经验
	dailyrecharge.onRecharge(actor,conf.useArg.recharge*count) -- 触发日常奖励
	return use_item_error_code.use_succeed
  end
end

local function useLvUp(actor, item_id, count)
	local ret = useDefault(actor,item_id,count) 
	if ret ~= use_item_error_code.use_succeed then 
		return ret
	end
	...
	for i=1,count do
		local level = LActor.getLevel(actor)
		if level < useArg.lv and level < #ExpConfig then -- 升级
			actorexp.confirmExp(actor, useArg.lv, LActor.getExp(actor), 0) -- 设置等级
			for i= level + 1, useArg.lv do -- 触发每隔等级的事件
...
--[[
    @desc: 增加跨服中boss的剩余归属次数,包含所有跨服boss:跨服BOSS 精英BOSS 3暗殿BOSS 4主宰BOSS
    author:{author}
    time:2020-04-16 11:13:46
    --@actor:
	--@itemId:
	--@count: 
    @return:
]]
local function useAddCrossSrvBelongTimes(actor,itemId, count)
	local conf = ItemConfig[itemId]
	local useArg = conf.useArg
	if(not useArg.bossType) then return end
	local useCount = 0
	if((useArg.bossType or 0) == 3) then
		useCount = darkhallbosssystem.itemAddBossBelongLeftCount(actor, itemId, count)
	end
	if((useArg.bossType or 0) == 4) then
		useCount = zhuzaibosssystem.itemAddBossBelongLeftCount(actor, itemId, count)
	end
	if(useCount>0) then
		return use_item_error_code.use_succeed
	end
...
local function useItemAddDoubleTime(actor,itemId,count)
	local conf = ItemConfig[itemId]
	local useArg = conf.useArg
	local time = useArg.time * count
	LActor.costItem(actor, itemId, count, "useItemAddDoubleTime")
	equipsystem.useItemAddDoubleTime(actor, time)
	return item.use_item_error_code.use_succeed
end

local function useResetAllRecord(actor,itemId,count)
	LActor.costItem(actor, itemId, count, "useResetAllRecord")
	local nextResetTime = System.getNowTime()
	rechargeitem.resetRecord(actor,nextResetTime)
	return use_item_error_code.use_succeed
end
use_item_func = 
{
	...
	[13] = useAddPrivilegeMonthCard, -- 使用增加特权月卡
	[14] = useAddVipExp, -- 使用增加vip经验
	[15] = useAddBossScore, -- 使用增加boss积分
	[16] = useAddFlagCount,-- 使用增加跨服BOSS中旗帜采集次数
	[17] = useAddCBossCount,-- 使用增加跨服BOSS挑战次数
	[18] = useAddDevilBossCount, -- 使用增加恶魔入侵BOSS挑战次数
	[19] = useVoucher, -- 使用代金券
	[20] = useLvUp,--直升80
	[21] = useAddCrossSrvBelongTimes, -- 增加跨服中boss的剩余归属次数,包含所有跨服boss:1 跨服BOSS 2 精英BOSS 3 暗殿BOSS 4主宰BOSS
	[22] = useItemAddDoubleTime,
	[23] = useResetAllRecord, -- 双倍充值重置道具
}
check_use_item_func = 
{
	...
	[14] = checkDefault,
	[15] = checkDefault,
	[16] = checkBelongItem, -- 跨服旗帜采集道具数量检测
	[17] = checkBelongItem, -- 跨服BOSS归属道具数量检测
	[18] = checkDeviBossItem, -- 增加恶魔入侵BOSS道具数量检测
	[19] = checkDeviBossItem, -- 代金券(使用跟devilboss一样的检测机制)
	[20] = checkDefault,
	[21] = checkDefault, -- 跨服里的BOSS用
	[22] = checkDefault,
	[23] = checkDefault,
}
