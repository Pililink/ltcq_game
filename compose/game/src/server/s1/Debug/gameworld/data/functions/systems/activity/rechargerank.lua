-- 消费排行榜
module("rechargerank", package.seeall)

--排行榜的常量定义
local rankingListNameTmp = "RechargeRank%d"
local rankingListFileTmp = "RechargeRank%d.rank"
local rankingListMaxSize = 100
local rankingListBoardSize = 20
local rankingListColumns = { "name" }
local activityIdSets = {} -- 开启中的充值活动id，其他id不存 dmw
...
local function updateDynamicFirstCache(actor_id)
    ...
    if #rankTbl ~= 0 then
        ...
        if actor_id == nil or actor_id == Ranking.getId(prank) then
            morship.updateDynamicFirstCache(Ranking.getId(prank), RankingType_Recharge)
        end
    end
end
]]
...
function resetRankingList(activId)
    print("start to resetRankingList rechargerank")
    local rankingListName = string.format(rankingListNameTmp, activId)
    
    local rank = Ranking.getRanking(rankingListName)
    if rank == nil then return end
    Ranking.clearRanking(rank)
    print("end to resetRankingList rechargerank")
end
...
function sendDaBiaoData(npack, actor, id, rankType, conf, index)
    ...
    local recharge = record.data.recharge or 0
    ...
    for i = 1, #rankTbl do
        ...
        LDataPack.writeData(npack, 4,
            ...
            dtInt, Ranking.getPoint(prank)      --recharge
        )
    end
end
...
local function onRecharge(id, conf)
	return function(actor, count)
		if activitysystem.activityTimeIsEnd(id) then return end
		local record = activitysystem.getSubVar(actor, id)
		record.data.recharge = (record.data.recharge or 0) + count
		updateRankingList(actor, id, record.data.recharge)
	end
end

--由对应的活动调用初始化,目前只有类型4用到了
function init(id, conf)
    if conf[0].rankType == RankingType_Recharge then
