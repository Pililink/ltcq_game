...
local function onLogin(actor)
    local var = getStaticData(actor)
    if ChongZhiBaseConfig.tenyuanRest then
        if (var.flag == nil) or (var.flag < ChongZhiBaseConfig.tenyuanRest) then 
            var.rewardDays = nil;
            var.rechargeDays = nil;
            var.isRecharge = nil;
            var.flag = ChongZhiBaseConfig.tenyuanRest or 0;
        end
    end
...
function remedy_isnoget(actor)
    local var = getStaticData(actor)
    return (var.rechargeDays or 0) <= 0
end

function remedy_onrecharge(actor)
    local var = getStaticData(actor)
    var.isRecharge = 1
    var.rechargeDays = (var.rechargeDays or 0) + 1
    sendRechargeDaysInfo(actor)
    print("remedy_onrecharge : " .. LActor.getActorId(actor))
end

function test(actor, index)
end

--@setrechargedaysawards 15 设置已充值天数15天
local gmsystem    = require("systems.gm.gmsystem")
local gmHandlers = gmsystem.gmCmdHandlers
gmHandlers.setrechargedaysawards = function(actor, args)
    local num = args[1]
    local var = LActor.getStaticVar(actor)
    var.rechargeDays = num 
    sendRechargeDaysInfo(actor)
end

gmHandlers.getrechargedaysawards = function(actor, args)
    local num = args[1]
    local var = LActor.getStaticVar(actor)
    LActor.sendTipmsg(actor, "当前已累计的充值天数:" .. var.rechargeDays)
end
