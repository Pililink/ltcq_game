	...
	for id,config in pairs(DailyConfig) do
		repeat
			...
			if config.param then
				local can = false
				for _,v in ipairs(config.param) do
					if param == v then
						can = true
					end
				end
			...
			if (bUpdate) then
				...
				dailyInfoSync(actor, id, dailyTaskVar.curValue, dailyTaskVar.status) -- 发送数据
...
function updateTaskValue(taskType, dailyTaskVar, value)
	if (taskcommon.getHandleType(taskType) == taskcommon.eAddType) then -- 27类型是该处
...
function onNewDay(actor, login)
	...
	for id,cfg in pairs(DailyConfig) do
		dailyTaskReset(id, dailyVar) -- 所有日常重置
		--通关关卡副本类型的
		if cfg.type == taskcommon.taskType.emFinishTypeDup and cfg.param[1] == 1 then
			local data = chapter.getStaticData(actor)
			if ChaptersConfig[data.level + 1] == nil then --所有关卡都通关了
				updateDailyTask(actor, cfg.type, cfg.param[1], cfg.target)
			end
		end
	end
