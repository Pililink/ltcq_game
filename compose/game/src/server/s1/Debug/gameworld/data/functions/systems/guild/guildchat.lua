...
function handleChatContent(actor, content)
	...
	if aData and ChatConstConfig.guildpaygoldnum then
		if aData.recharge < ChatConstConfig.guildpaygoldnum then
			local tips = string.format( Lang.ScriptTips.chat001, ChatConstConfig.guildpaygoldnum )
	...
	for i = 1, #(actors or {})  do
		local pack = LDataPack.allocPacket(actors[i], systemId, Protocol.sGuildCmd_Chat) -- 37-26
	...
	--记录公会聊天监控
	chat.addChatMonitorGuild(actor,LGuild.getGuildId(guild),content)
...
-- 发送帮派公告
function sendNotice(guild, content, notLogFlag) -- notLogFlag:不记录到日志
	local pack = LDataPack.allocPacket()
...
-- 发送工会消息清除消息 37-33
local function sendClearMsgToGuild(guildid)
	local actorlist = System.getOnlineActorList()
	if actorlist then
		for i=1,#actorlist do
			if actorlist[i] and tostring(LActor.getGuildId(actorlist[i])) == guildid then
				local pack = LDataPack.allocPacket(actorlist[i], Protocol.CMD_Guild, Protocol.sGuildCmd_GuildChatMsgDelGM) -- 37-33
				LDataPack.flush(pack)
			end
		end
	end
...
--公会聊天监控简单粗暴点，把聊天数据全部删掉
function clearChatLog(guildid)
	local guild = LGuild.getGuildById(guildid)
	if guild ~= nil then
		LGuild.clearChatLog(guild)
		sendClearMsgToGuild(guildid) -- 通知客户端
	end
end
...
-- 删除工会所有
-- @delguildchat 123
gmHandlers.delguildchat = function(actor, args)
	local guildid = args[1] -- 工会id
	clearChatLog(guildid)
	return true
end
