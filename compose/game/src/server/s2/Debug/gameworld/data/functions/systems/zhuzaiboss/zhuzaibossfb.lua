-- 主宰boss管理模块(跨服服)
module("zhuzaibossfb", package.seeall)
-- 日志
local function LOG(actor,errlog)
	local actorid = LActor.getActorId(actor)
	print("[ERROR]:zhuzaibossfb." .. errlog .. " actorid:" .. actorid)
end

local function DEBUG(actor,log) -- 上线后把内容注掉
	local actorid = LActor.getActorId(actor)
	print("[DEBUG]:zhuzaibossfb." .. log .. " actorid:" .. actorid)
end

local function LOGINFO(actor,log)
	local actorid = LActor.getActorId(actor)
	print("[INFO]:zhuzaibossfb." .. log .. " actorid:" .. actorid)
end

local function SYSINFO(log)
	print("[SYSINFO]:zhuzaibossfb." .. log)
end

local function SYSLOG(log)
	print("[SYSERROR]:zhuzaibossfb." .. log)
end

local function SYSDEBUG(log) -- 上线后把内容注掉
	print("[SYSDEBUG]:zhuzaibossfb." .. log)
end
...
local config = ZhuZaiBossConfig
globalZhuzaiBossData = globalZhuzaiBossData or {}
...
function getSysData()
	...
	if nil == var.zhuzaiboss then var.zhuzaiboss = {} end
	if nil == var.zhuzaiboss.bossList then var.zhuzaiboss.bossList = {} end
	return var.zhuzaiboss
end
...
--发送boss信息到游戏服 7- 13精英BOSS刷新通知 s2s
...
local function sendBossInfo(id, srvId)
	if not System.isCommSrv() then -- 跨服
		...
		LDataPack.writeByte(npack, CrossSrvSubCmd.SCBossCmd_RefreshZhuZaiBoss) -- 7 - 13 主宰BOSS
...
local function canGetBelong(actor, type)
	...
	if rewardType.bossReward == type then
		return (var.bossBelongLeftCount or ZhuZaiBossBase.zhuzaibossBelongCount) > 0
	else
...
local function reduceBelongCount(actor, type)
	...
	if rewardType.bossReward == type then -- 减少boss归属
		var.bossBelongLeftCount = (var.bossBelongLeftCount or ZhuZaiBossBase.zhuzaibossBelongCount) - 1
...
local function notifyRebornTime(actor, killerHdl)
    ...
    local npack = LDataPack.allocPacket(actor, Protocol.CMD_CrossBoss, Protocol.sCrossBossCmd_ResurgenceZhuzaiInfo)
...
local function reborn(actor, id)
	...
	local x, y = crosselitebosssystem.getRandomPoint(ZhuZaiBossConfig[id])
...
local function refreshBossTimer(id)
	...
	if not conf then 
		SYSLOG("zhuzaibossfb.refreshBossTimer:conf nil, id:"..tostring(id)) 
		return 
	end

	local fbInfo = getBossData(id)
	if not fbInfo then 
		SYSLOG("zhuzaibossfb.refreshBossTimer:fbInfo nil, id:"..tostring(id)) 
		return 
	end
	...
	if ins then
		...
		print("zhuzaibossfb.refreshBossTimer: refresh monster success, id:"..tostring(id))
	end
end
...
local function sendReward(actor, reward, rewardType, id, isBelong)
	...
	local npack = LDataPack.allocPacket(actor, Protocol.CMD_CrossBoss, Protocol.sCrossBossCmd_SendZhuzaiRewardInfo) -- 72-70发送奖励
	...
	if not System.isCommSrv() then
		...
		LDataPack.writeByte(npack, CrossSrvCmd.SCCrossBossCmd)
		LDataPack.writeByte(npack, CrossSrvSubCmd.SCBossCmd_sendZhuZaiReward) -- 7-14 发送奖励
	...
	print("zhuzaibossfb.sendReward:send success, isBelong:".. (isBelong and 1 or 0) ..", actorId:"..tostring(LActor.getActorId(actor))
...
function sendBossBelongInfo(id, actor, oldBelong)
	...
	local npack = nil
    if actor then
        npack = LDataPack.allocPacket(actor, Protocol.CMD_CrossBoss, Protocol.sCrossBossCmd_belongZhuzaiUpdate) -- 72-66
    else
        npack = LDataPack.allocPacket()
		LDataPack.writeByte(npack, Protocol.CMD_CrossBoss)
        LDataPack.writeByte(npack, Protocol.sCrossBossCmd_belongZhuzaiUpdate)
    end
...
local function clearBossBelongInfo(id, actor)
    ...
    if nil == bossData then print("zhuzaibossfb.clearBossBelongInfo:bossData is null, id:"..id) return end
    ...
    if actor == bossData.bossBelong then
		...
		--无归属回血
		if ZhuZaiBossBase.revivalTime and not bossData.revEid then
			bossData.revEid = LActor.postScriptEventLite(nil, ZhuZaiBossBase.revivalTime * 1000, function(_, bid)
...
--进入公告
local function sendEnterNoticeId(actor, id) -- 7-15
	local data = getBossData(id)
	--发公告
	local npack = LDataPack.allocPacket()
	LDataPack.writeByte(npack, CrossSrvCmd.SCCrossBossCmd)
	LDataPack.writeByte(npack, CrossSrvSubCmd.SCBossCmd_enterZhuZaiFb)
...
local function notifyShield(hfuben, curshield,maxShield)
    local npack = LDataPack.allocPacket()
    LDataPack.writeByte(npack, Protocol.CMD_CrossBoss)
	LDataPack.writeByte(npack, Protocol.sCrossBossCmd_ZhuzaiBossShield)
...
local function onBossDamage(ins, monster, value, attacker, res)
	...
	local conf = ZhuZaiBossConfig[ins.data.id].shield[1]
...
local function onActorDie(ins, actor, killerHdl)
    ...
    if not et then print("zhuzaibossfb.onActorDie:et is null") return end

    local bossData = getBossData(ins.data.id)
    if nil == bossData then print("zhuzaibossfb.onActorDie:bossData is null, id:"..ins.data.id) return end
    ...
    --复活定时器
    local var = getCrossStaticData(actor)
	var.resurgence_cd = System.getNowTime() + ZhuZaiBossBase.rebornCd
	var.rebornEid = LActor.postScriptEventLite(actor, ZhuZaiBossBase.rebornCd * 1000, reborn, ins.data.id)
...
local function sendBossData(actor)
	...
	if actor then
		...
		npack = LDataPack.allocPacket()
		LDataPack.writeByte(npack, Protocol.CMD_CrossBoss)
		LDataPack.writeByte(npack, Protocol.sCrossBossCmd_SendZhuzaiBossInfo)
	end
...
local function onMonsterDie(ins, mon, killerHdl)
    ...
    if monId ~= bossId then
		print("zhuzaibossfb.onMonsterDieP:monid("..tostring(monId)..") ~= bossId("..tostring(bossId).."), id:"..ins.data.id)
		return
	end
	
	local conf = ZhuZaiBossConfig[ins.data.id]
...
local function onReqBuyCd(actor, packet)
    ...
    if ZhuZaiBossBase.rebornCost + (data.needCost or 0) > yb then
    ...
    if System.isCommSrv() then
		LActor.changeYuanBao(actor, 0 - ZhuZaiBossBase.rebornCost, "servercrossboss buy cd")
	else
		data.needCost = (data.needCost or 0) + ZhuZaiBossBase.rebornCost
	end
...
local function initBossDataFb()
	-- 创建精英BOSS副本
	for i=1, #ZhuZaiBossConfig do -- 精英BOSS
		createBossFb(ZhuZaiBossConfig[i]) -- 跨服精英BOSS 这里写-i是为了发公告的时候好确认是哪个精英boss 大于0的数字已经被跨服BOSS用了,0是苍月岛
	end
end

--活动开始
-- createflag 需要创建标志 true:需要创建 false:不要创建
function zhuzaiBossOpen(createflag)
	SYSINFO("zhuzaiBossOpen start")
	-- 开启战斗服boss
	if System.isCommSrv() then -- 普通服不开启
		SYSINFO("zhuzaiBossOpen error,common srv can not open eliteboss") 
		return 
	end
	local initFlag = getGlobalData().initflag
	local data = getSysData()
	if(initFlag) then
		SYSINFO("zhuzaiBossOpen has been initialized")
		return
	end
	...
	for srvId, time in pairs(srvInfo or {}) do -- [id] = 开服时间
		if (ZhuZaiBossBase.zhuzaibossopenDay or 0) > System.getTimeToNowDay(time)+1 then -- 开服days dmw
			SYSLOG("zhuzaiBossOpen:time not enough,srvId:" .. srvId)
			return
		end
	end
...
local function initGlobalData()
	...
	SYSDEBUG("initGlobalData DMWFLAG 主宰BOSS初始化 START")
	--注册副本事件
	 for _, conf in pairs(ZhuZaiBossConfig) do -- 精英BOSS
    ...
    netmsgdispatcher.reg(Protocol.CMD_CrossBoss, Protocol.cCrossBossCmd_BuyZhuzaiCd, onReqBuyCd) -- 角色花钱复活
	netmsgdispatcher.reg(Protocol.CMD_CrossBoss, Protocol.cCrossBossCmd_CancelZhuzaiBelong, onCancelBelong) -- 取消归属
	...
	SYSDEBUG("initGlobalData DMWFLAG 主宰BOSS初始化 OVER")
end
...
gmHandlers.zhuzaiboss = function(actor, args)
