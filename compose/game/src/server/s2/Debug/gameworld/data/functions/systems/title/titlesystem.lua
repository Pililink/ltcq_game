...
titleData={ -- 称号数据
    titles={ -- 称号列表
        [index]={ -- 称号id(非物品id)
            ...
            id, -- 角色id
            titleid, -- 称号id
            curlv, -- 称号当前等级 add by dmw 2019-11-11
            ...
        }
        [index2]={
        }
...
local function updateAttr(actor)
    ...
    for k,v in pairs(conf) do
        ...
        if var.titles[k] then
            local titlelv = var.titles[k].curlv or 1
            if(titlelv > 1) then
                v = TitleLevelUp[k][titlelv] -- 升级过的称号，属性看TitLeLevelUp表
            end
...
function addTitle(actor, tId)
    ...
    if tConf == nil then
        if(tId ~= nil) then
            print(LActor.getActorId(actor) .. "titlesystem.addTitle " .. tId .. " fail,TitleConf not exist,need compensation for the player")
        else
            print(LActor.getActorId(actor) .. "addTitle tConf is nil")
        end
        return 
    end
    ...
    if isChange then
        ...
        LDataPack.writeInt(pack,var.titles[tId].curlv or 1) -- 该称号等级 dmw
...
function getTitlesInfo(actor, pack)
    ...
    for k,v in pairs(conf) do
        if var.titles[k] then
            ...
            LDataPack.writeInt(npack, var.titles[k].curlv or 1) -- 称号当前等级 dmw
            count = count + 1
        end
    end
...
-- 升级角色称号
function upgradeRoleTitle(actor, pack)
    local titleId = LDataPack.readInt(pack) -- 称号id
    local var = getVar(actor)
    if(var == nil) then
        print("roleId:%d titlesystem getVar err",roleId)
        return
    end
    if(var.titles[titleId] == nil) then
        print("roleId:%d not exist title:%d",roleId,titleId)
        return
    end
    local titleData = var.titles[titleId] -- 相应称号id的称号信息
    -- 获取该角色对应称号现在的等级
    local nextlv = (titleData.curlv or 1) + 1
    -- 获取配置文件中升级需要的材料
    if(nextlv > (#TitleLevelUp)) then
        print("roleId:%d title:%d reach maximum level",roleId,titleId)
        return
    end
    local conf = TitleLevelUp[titleId]
    if(not conf) then
        print("upgradeRoleTitle conf is nil,titleId:%d",titleId)
        return
    end
    local costItemid = conf[nextlv - 1].cost.itemId -- 消耗道具id，策划配置中升级消耗写在了当前等级
    local costCount = conf[nextlv - 1].cost.num -- 消耗道具数量
    -- 判断材料够不够
	if not LActor.checkItemNum(actor,costItemid,costCount,false) then 
		print(LActor.getActorId(actor) .. "upgradeRoleTitle, item not enough " .. costItemid)
		return -- 数量不够
	end
    -- 扣除材料后升级
    local bRet = LActor.consumeItem(actor, costItemid, costCount, false,"upgradeRoleTitle:" .. costItemid) -- 先扣道具
	if(bRet == false) then
		print("error")
		return -- 扣除失败了也显示不可使用
    end
    -- 设置称号等级
    titleData.curlv = nextlv
    -- 重新计算角色属性数据，属性要重新修改内容，读取额外的配置
    updateAttr(actor)
    -- 反馈客户端本次升级结果
    
    local pack = LDataPack.allocPacket(actor, Protocol.CMD_Title, Protocol.sTitleCmd_LevelUp) -- 称号升级结果
    if pack == nil then return end
    LDataPack.writeInt(pack, titleId) -- 称号id
    LDataPack.writeInt(pack, 0) -- 错误码 0正常,异常不会走到这一步直接return了
    LDataPack.writeInt(pack, nextlv) -- 升级后等级
    LDataPack.flush(pack) -- 发包
end

function isTitleAcquired(actor,titleId) -- 是否已获得某称号 titleId：称号id
    local var = getVar(actor)
    if(var and var.titles and var.titles[titleId]) then
        return true
    end
    print(LActor.getActorId(actor) .. " check:title not acquired,titleid=" .. titleId)
    return false
end
...
netmsgdispatcher.reg(Protocol.CMD_Title, Protocol.cTitleCmd_LevelUp, upgradeRoleTitle) -- 升级角色称号
