...
function finishAchieveTask(actor, id, taskId)
    ...
    print(LActor.getActorId(actor).." achievetask.finishAchieveTask, achieveId:"..id..", taskId:"..achieveVar.taskId..", curValue:"..achieveVar.curValue..", status:"..achieveVar.status)
	achieveInfoSync(actor, id, achieveVar.taskId, achieveVar.curValue, achieveVar.status)
    ...
    print(LActor.getActorId(actor).." next achievetask.finishAchieveTask, achieveId:"..id..", taskId:"..achieveVar.taskId..", curValue:"..achieveVar.curValue..", status:"..achieveVar.status)
end

-- 这里是为了修复成就任务的一个bug（校准勋章跟成就任务）
-- 不知道哪里修改了taskid，导致taskid比勋章的偏低1级，
-- 这里每次登录时做特殊处理；
-- 注意：achieveId, taskId 为当前勋章上一级的配置id；
function checkAndFinish(actor, achieveId, taskId)
    local data = getStaticData(actor)
    local achieveVar = data[achieveId]
    if achieveVar == nil then return end
    local configs = taskcommon.getAchieveConfig()
    if configs[achieveId] == nil then return end

    -- 没问题的不处理了
    if achieveVar.taskId > taskId then return end

    -- 执行处理
    local nextTaskId = getNextId(achieveId, achieveVar.taskId)
    if nextTaskId > achieveVar.taskId then
        data[achieveId] = initTask(actor, configs[achieveId], nextTaskId)
        acceptAchieveTaskSync(actor, achieveId, nextTaskId, achieveVar.curValue, achieveVar.status)
    end
end
