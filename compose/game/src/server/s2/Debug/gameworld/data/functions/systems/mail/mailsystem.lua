...
function giveAward(actor, uid, tAwardList)
	...
	for _,tb in pairs(tAwardList) do
		...
		if (nType == AwardType_Item) then
			...
			if config and ((config.type == ItemType_Equip) or (config.type == ItemType_WingEquip) or (config.type == ItemType_TogetherHit) or (config.type == ItemType_EquipRandom)) then
