...
--net end
local function onFirstLoginSendMail(actor, first)
	if first == 0 then
		print(LActor.getActorId(actor).."not first login")
		return;
	end
	if FirstLoginMailConfig == nil then
		print(LActor.getActorId(actor).."no FirstLoginMailConfig")
		return;
	end
	local mailData = FirstLoginMailConfig
    mailsystem.sendMailById(LActor.getActorId(actor), mailData)
end


local function onLogin(actor,first)
...
--	getLoginRewards(actor,1)
	...
	onFirstLoginSendMail(actor, first)
	--sendLoginRewardData(actor)
end
