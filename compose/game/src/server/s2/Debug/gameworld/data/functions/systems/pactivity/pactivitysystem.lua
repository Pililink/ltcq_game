...
-- 注册活动开启时的回调
function regOpenActivityFuncs( type, fn )
	openActivityFuncs[type] = fn
end
...
local function openActivity( actor, id, cfg )
	...
	if openActivityFuncs[cfg.activityType] then
		local record = getSubVar(actor, id)
		openActivityFuncs[cfg.activityType](id, confList[cfg.activityType], actor, record)			
	end
