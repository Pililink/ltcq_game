    ...
    if info and info.damagelist then
		for k, v in pairs(info.damagelist) do
			...
			if actor then 
				...
				elseif conf.type == Type.worldBoss or conf.type == Type.DarkMjBoss then--秘境boss
				...
				asynevent.reg(k,function(imageActor,srcActorId)
...
function AddBelongCount(actor, type, count)
	local data = getStaticData(actor)
	local canCount = baseConf.belongCount[type] or 0
	if not data.can_belong_count then data.can_belong_count = {} end
	if not data.can_belong_count[type] then data.can_belong_count[type] = canCount end
	
	data.can_belong_count[type] = data.can_belong_count[type] + count
	sendBossViewData(actor,type)
end
