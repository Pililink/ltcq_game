...
function activeremedy(actor, val)
	local id = 324
	local conf = ActivityType3Config[id]
	local var = activitysystem.getSubVar(actor,id)
	var.cumulation = (var.cumulation or 0) + val
	print("324补偿" .. "目前" .. var.cumulation .. "增加了" .. val)
	activitysystem.sendActivityData(actor, id)
end

local gmsystem    = require("systems.gm.gmsystem")
local gmHandlers = gmsystem.gmCmdHandlers
-- @addremedyrecharge 324 10000 给324活动增加10000充值额度
gmHandlers.addremedyrecharge = function(actor, args)
	local id = tonumber(args[1]) -- 活动id
	local val = tonumber(args[2]) -- 充值额度
	local var = activitysystem.getSubVar(actor,id)
	var.cumulation = (var.cumulation or 0) + val
	activitysystem.sendActivityData(actor, id)
end

-- @showrecharge 324 显示324的额度
gmHandlers.showrecharge = function(actor, args)
	local id = tonumber(args[1]) -- 活动id
	local var = activitysystem.getSubVar(actor, id)
	LActor.sendTipmsg(actor, "活动id:" .. id .. " 已充值额度" .. (var.cumulation or 0), 4)
end

--@setdayrecharge 342 1 1 -- 设置索引1 连充天数为1 只适用于连充活动,一般来说索引和天数相同即可
gmHandlers.setdayrecharge = function(actor, args)
	local id = tonumber(args[1]) -- 活动id
	local k = tonumber(args[2]) -- key
	local state = tonumber(args[3]) -- 满足的天数
	local var = activitysystem.getSubVar(actor, id)
