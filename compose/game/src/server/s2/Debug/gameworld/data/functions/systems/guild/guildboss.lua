...
local function reqGetAllInfo(actor, packet)
	...
	LDataPack.writeInt64I(npack, gdata and gdata.boss_hurt or 0) --boss收到的伤害
...
local function reqGetRankInfo(actor, packet)
	...
	if gcount > 0 then
		...
		for i = 1,gcount do
			...
			LDataPack.writeInt64I(npack, grd[i].damage)
		end
	end
	--写入个人排名数据
	local pdrank = nil
	...
	if pdrank then
		...
		for k,rank_data in ipairs(pdrank) do
			...
			LDataPack.writeInt64I(npack, rank_data.damage)
...
-- @guildbosssetbossid 2
gmHandlers.guildbosssetbossid = function(actor, args)
