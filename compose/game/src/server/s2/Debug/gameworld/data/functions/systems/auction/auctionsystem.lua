...
auction = { -- 全局数据 dmw
    AuctionRecord = { 
	    {
	            serialId = 123, --拍卖品序列号,用于确认拍卖行中的拍卖品
		    auctionId = 999,  --拍卖品id,根据拍卖品id查表获取itemid
		    name = "abc",     --成交者名字
	...
	autonum = 0, -- 自增数字,拍卖品序列号的一部分
	ownerrecords={ -- 拍卖品收益者名单,最大200条
		[serial] = {123 456 789} -- 拍卖品序列号 收益者名单
		...
	}
	    ...
    }
...
local function getStaticData(actor) -- 个人变量
...
local function getGlobalData() -- 全局变量
...
--增加成交记录
-- auctionId:拍卖品id name:购买者名字 txType:0 流拍 1 竞价 2一口价 price:成交价 guildId:购买者工会id serialId:拍卖品序列号
local function addRecord(auctionId, name, txType, price, guildId,serialId)
	local gdata = getGlobalData()
	gdata.AuctionRecord = gdata.AuctionRecord or {}
	local AuctionRecord = gdata.AuctionRecord
...
local function addOwnerRecord(serialid, owners)
	if(type(owners) ~= "table") then print("auctionsystem.addOwnerRecord owners is not table") return end
	local globalauction = getGlobalData()
	globalauction.ownerrecords = globalauction.ownerrecords or {}
	if(#globalauction.ownerrecords > AuctionConfig.successRecordMax) then table.remove(globalauction.ownerrecords, 1) end
	globalauction.ownerrecords[serialid] = {}
	for _,v in pairs(owners) do
		table.insert(globalauction.ownerrecords[serialid], v)
		-- print("玩家id 插入" .. v)
	end
	
end

--物品上架 auctionId auctionId: 拍卖品id  看配置表,一个拍卖品id对应一个itemid
function addGoods(owners, guildId, auctionId, isPersonal)
	...
	addOwnerRecord(id, owners) -- 添加拍卖品收益者记录
...
--写一个可以出售的商品的数据
-- aid:actorid guildId:工会id goodsType:工会拍卖/全服拍卖 pack:发送用的数据包 gdata:全局数据
...
--购买商品
local function buyGoods(actor, id, goodsType) -- id 拍卖品序列号
	...
	if ret == ResultNo.success then
		...
		addRecord(gdata.auctionId, LActor.getActorName(LActor.getActorId(actor)), TxType.buy, aitemConf.buy, (goodsType == GoodsType.guild) and LActor.getGuildId(actor) or 0,id)
		local genus = string.format("%s_%s", aitemConf.bid, aitemConf.buy)
...
local function onReqGoodsList(actor, packet)
	local goodsType = LDataPack.readChar(packet) -- 工会拍卖/全服拍卖
	local goodsCount = 0
	local nowTime = System.getNowTime()
	local actorId = LActor.getActorId(actor) -- 角色id
	local guildId = LActor.getGuildId(actor) -- 工会id

	local pack = LDataPack.allocPacket(actor, Protocol.CMD_Auction, Protocol.sAuctionCmd_RepGoodsList)
	if not pack then return end
	LDataPack.writeChar(pack, goodsType) -- 工会拍卖/全服拍卖
	local pos1 = LDataPack.getPosition(pack)
	LDataPack.writeInt(pack, goodsCount) -- 商品数量
...
local function onReqBid(actor, packet)
	local id = LDataPack.readInt(packet) -- 序列号
	local goodsType = LDataPack.readChar(packet) -- 物品类型 工会/全服
...
local function onReqRecordList(actor, packet)
	...
	local pos1 = LDataPack.getPosition(pack)
	LDataPack.writeShort(pack, count) -- changdu
	for _,v in pairs(AuctionRecord) do
		if guildId == v.guildId then
			-- 序列号加入
			LDataPack.writeInt(pack, v.serialId or 0) -- 拍卖品序列号(确认拍卖品)
			LDataPack.writeInt(pack, v.auctionId) -- 拍卖品id(能确认到拍卖品的物品id)
...
-- 请求展示某拍卖品的收益者 76-8
local function onShowOwnwers(actor, packet)
	local id = LDataPack.readInt(packet) -- 拍卖序列号
    if(not id) then print("AuctionGoodsList.onShowOwnwers id is nil") return end

	local gauction = getGlobalData()
	if(not gauction.ownerrecords) then print("AuctionGoodsList.onShowOwnwers gauction.ownerrecords is nil") return end
	owners = gauction.ownerrecords[id] or {}

	local pack = LDataPack.allocPacket(actor, Protocol.CMD_Auction, Protocol.sAuctionCmd_ShowOwners)
	local nCount = #owners
	--print("发送收益者数量" .. nCount)
	LDataPack.writeInt(pack, nCount) -- 收益者数量
	for _,actorid in pairs(owners) do
		local name = LActor.getActorName(actorid)
		LDataPack.writeString(pack, name or "") -- 收益者名称
		--print("收益者:" .. name)
	end
	LDataPack.flush(pack)
end
...
-- 76-7 被占用了
netmsgdispatcher.reg(Protocol.CMD_Auction, Protocol.cAuctionCmd_ShowOwners, onShowOwnwers)  --协议 76-8
