...
include 'config.php';
	...
	$status= $row['status'];
	if ($_SESSION['check'] <> $check || empty($_SESSION['status']) || $_SESSION['status'] <> $status  || $_SESSION['status'] <> 'vip3'){				
	unset($_SESSION);
...
<h1><?php echo $_SESSION['gmbt'];?>玩家后台</h1>
 <h3 style="color:blue"><?php echo $_SESSION['name'];?> 欢迎登陆</h3> 
  <label style="color:red" for="v">您的账号到期时间：<?php echo $_SESSION['endtime'];?></label> <br>
	 <button class="btn btn-info btn-block" onclick="window.location.href='player3.php'">充值系统</button>
	 <button class="btn btn-info btn-block" onclick="window.location.href='playermail3.php'">邮件系统</button>
	 <button class="btn btn-info btn-block" onclick="window.location.href='playerpass3.php'">修改密码</button>
...
<form id="form1" name="form1" method="post" action="playerquery.php">
    ...
    <div class="form-group">
		...
		<?php
        $file = fopen("onekey/item.vip3.txt", "r");
        while(!feof($file))
        {
            $line=fgets($file);
			$txts=explode(';',$line);
