...
<h1><?php echo $_SESSION['gmbt'];?>授权后台</h1>
 <h3 style="color:blue"><?php echo $_SESSION['name'];?> 欢迎登陆</h3> <br>
	...
	<button class="btn btn-info btn-block" onclick="window.location.href='exit.php'">登陆注销</button>	
	 <table style='text-align:margin：auto;' class="table table-bordered" border='1'width="40%"> <tr><th style='text-align: center;'>用户名</th><th style='text-align: center;'>角色名</th><th style='text-align: center;'>权限</th><th style='text-align: center;'>分区</th></th><th style='text-align: center;'>绑定角色ID</th><th style='text-align: center;'>有效时间</th></th><th style='text-align: center;'>改/删</th></tr> 
	<?php
			//引用conn.php文件
        require 'config.php';
        //查询数据表中的数据
		...
		$result=mysql_query("select * from user where  status NOT IN('proxy','admin') and addname = '$user'  limit $startCount,$perNumber" ); //根据前面的计算出开始的记录和记录数
		while ($row=mysql_fetch_array($result)) {
		echo "<tr><td>$row[1]</td><td>$row[3]</td><td>$row[4]</td><td>$row[5]区</td><td>$row[6]</td><td>$row[7]</td><td><a href='proxyedit.php?id=$row[0]'>点击</a></td></tr>";
		}				
		if ($page != 1) { //页数不等于1
		?>
  ...
  <p class="admin_copyright">   <?php echo $_SESSION['copyright'];?></p>  
 </div>
</div>
</body>
</html>